...
Lemma 3.2.5, discrete case).  If an orthogonal representation of a group
generated by the finite set `Q` has almost-invariant unit vectors but no
nonzero invariant vectors, then for every `M > 0` there is a vector of
displacement exactly one all of whose `M`-neighbours have displacement
more than one half.  A counterexample would halve displacement within a
scale-invariant distance; iterating produces a geometric Cauchy sequence
whose limit is a nonzero invariant vector. -/
theorem exists_displacement_one_of_almost_invariant [CompleteSpace E]
  ...
  have step : ∀ ζ : E, 0 < displacement ρ Q hQ ζ →
    ...
    · rw [displacement_smul, abs_of_pos hζ, hFt']
      ring
  -- Start from an almost-invariant unit vector of displacement below
  -- `1 / (4 * M)`; its displacement is positive since it is not zero.
