...
- [ ] Prove the GE/`K₁` inputs and `GL_r(L) = EL_r(L)` for all required
  ...
  - [ ] B4. `K₁(L) = 0` in the form used: `diag(u,1) ∈ EL₂(L)` for every
    ...
    `X = [[s_w, 1−p_w],[0, t_w]]`.  Remaining: the Leavitt-side layer
    (the `X`-conjugation identity itself, and the scalar collapse
    `c̄ = c̄²` via `κ₀(u)·κ₁(u) = s₀ut₀ + s₁ut₁`), and the irreducible
    input that `L^×/H` is generated by scalars, i.e. the rose-graph
    `K₁` computation — the manuscript's cited
    `[AraBrustengaCortinas09]` input, and also the load-bearing input
    of the cited `[KhanhThanh26]` generation theorem
